  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-teal-50 to-blue-50 flex items-center justify-center">
        ...
        >
          <h1 className="text-4xl font-bold text-teal-700 mb-4">ChatBot Portal</h1>
          <motion.div
            animate={{
              rotate: 360,
              scale: [1, 1.1, 1],
      ...
      <AnimatePresence>
        {showError && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-red-100 border border-red-400 text-red-700 px-6 py-3 rounded-full shadow-lg"
          >
